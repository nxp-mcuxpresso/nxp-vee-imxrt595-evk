...
const clock_sys_pll_config_t g_sysPllConfig_BOARD_BootClockRUN = {
    ...
    .sys_pll_mult = kCLOCK_SysPllMult16  /* Divide by 22 */
};
...
void BOARD_BootClockRUN(void)
{
    ...
    BOARD_SetFlexspiClock(FLEXSPI0, 0U, 1U);
    ...
    /* Clock source configuration finished, power down FRO if not used. */
    CLKCTL0->FRODIVOEN &= ~CLKCTL0_FRODIVOEN_FRO_DIV8_O_EN_MASK;
    
    /* Stop some unused clocks that are enabled by default (likely by the ROM). */
    CLOCK_AttachClk(kNONE_to_OSTIMER_CLK);
    CLOCK_AttachClk(kNONE_to_32KHZWAKE_CLK);
    CLOCK_AttachClk(kDSP_PLL_to_DSP_MAIN_CLK); // DSP PLL is OFF by default. Force to use it instead of another enabled clock.

    /* Reset unused peripherals and stop clocks.*/
    RESET_SetPeripheralReset(kHASHCRYPT_RST_SHIFT_RSTn);
    RESET_SetPeripheralReset(kPUF_RST_SHIFT_RSTn);
    RESET_SetPeripheralReset(kRNG_RST_SHIFT_RSTn);
