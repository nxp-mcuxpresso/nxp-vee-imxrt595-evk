 ...
 * 1. Patch to consider frame buffer stride during DSI transfer
 */
...
status_t DC_FB_DSI_CMD_GetLayerDefaultConfig(const dc_fb_t *dc, uint8_t layer, dc_fb_info_t *fbInfo)
{
    ...
    fbInfo->strideBytes = VIDEO_GetPixelSizeBits(panelHandle->pixelFormat) / 8U;
...
status_t DC_FB_DSI_CMD_SetFrameBuffer(const dc_fb_t *dc, uint8_t layer, void *frameBuffer)
{
    ...
    // MicroEJ patch to consider frame buffer stride during transfer
    uint16_t fbInfo_width            = fbInfo->strideBytes / (VIDEO_GetPixelSizeBits(fbInfo->pixelFormat) / 8U);    
    status = MIPI_DSI_SelectArea(dsiDevice, fbInfo->startX, fbInfo->startY, fbInfo->startX + fbInfo_width - 1U,
